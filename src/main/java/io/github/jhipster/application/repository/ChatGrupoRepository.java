...
public interface ChatGrupoRepository extends JpaRepository<ChatGrupo, Long>, JpaSpecificationExecutor<ChatGrupo> {
    ...
    @Query("select chatGrupo from ChatGrupo chatGrupo where chatGrupo.de.login = ?#{principal.username}")
    List<ChatGrupo> findByDeIsCurrentUser();

    @Query("select chatGrupo from ChatGrupo chatGrupo where chatGrupo.para.login = ?#{principal.username}")
